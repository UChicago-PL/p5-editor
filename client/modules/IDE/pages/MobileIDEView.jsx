...
const MobileIDEView = (props) => {
  ...
  const [toggleNavDropdown, NavDropDown] = useAsModal(
    <Dropdown items={getNavOptions(username, logoutUser, toggleForceDesktop)} align="right" />
  );

  const [toggleExplorer, Explorer] = useAsModal(
    (toggle) => <MobileExplorer id={getRootFileID(files)} canEdit={false} onPressClose={toggle} />,
    true
  );
  ...
  return (
    <Screen fullscreen>
      <Explorer />
      <Header title={withChangeDot(project.name, unsavedChanges)} subtitle={filename}>
        <NavItem>
          <IconButton onClick={toggleNavDropdown} icon={MoreIcon} aria-label="Options" />
          <NavDropDown />
        </NavItem>
        <li>
          <IconButton
