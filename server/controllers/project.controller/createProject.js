...
export function apiCreateProject(req, res) {
  ...
  function checkUserHasPermission() {
    if (req.user.username !== req.params.username) {
      const msg = `'${req.user.username}' does not have permission to create for '${req.params.username}'`;
      console.log('no permission', msg);
      const error = new ProjectValidationError(msg);
      error.code = 401;
