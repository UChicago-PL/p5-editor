...
export function createSubmission(props) {
  const { files, project, username, submissionId } = props;
  return new Promise((resolve, reject) => {
    Submission.create(
      { files, username, project: project._id, projectName: project.name, submissionId },
      (err, item) => {
        if (err) {
  ...
  getFileContent(id).then((contents) => {
    ...
    new O({ auth: req.user.githubToken })
      .createPullRequest({
        ...
        body: 'Automatically generated PR request.',
        head: `autogenerated-pr-${submissionId}`,
        changes: [
          {
            files: prepPR(contents, toSubFolder.toLowerCase() === 'yes' ? `${name}/` : '', true),
      ...
      .then((result) => {
        ...
        return createSubmission({ username: ownerName, files: contents.files, project, submissionId });
      })
      .catch((err) => {
