...
export async function submitGHRepo(req, res) {
  ...
  const allowListItem = await UserAllowlist.findOne({ github: req.user.github });
  ...
  if (!edition) {
    console.log('error finding edition for', JSON.stringify({ edition, allowListItem }));
