...
export function getGHRepos(req, res) {
  // TODO might also consider jamming these into the session storage if available?
  if (!req.user || !req.user.github || !req.user.githubToken) {
    res.status(404).json({ success: false, message: 'You must be logged in to complete this action.' });
    return;
  }

  new Octokit({ auth: req.user.githubToken })
    .request('GET /user/repos?affiliation=owner')
    .then((result) => {
      res.status(200).json(
